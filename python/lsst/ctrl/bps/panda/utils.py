...
def create_idds_build_workflow(config_file, config, remote_build):
    ...
    # _LOG.info("remote build: %s" % remote_build.get('runnerCommand'))
    # remote_build = dict(remote_build)
    # executable = remote_build['runnerCommand'].strip()
    search_opt = {"replaceVars": False, "expandEnvVars": False, "replaceEnvVars": False, "required": False}
    executable = remote_build.search("runnerCommand", search_opt)[1]
    ...
    # TODO: fill other parameters based on config, such as site, prodSourceLabel, mem, core count and so on
    build_work = DomaPanDAWork(executable=executable,
                               task_type='lsst_build',
                               primary_input_collection={'scope': 'pseudo_dataset',
                                                         'name': 'pseudo_input_collection#1'},
                               output_collections=[{'scope': 'pseudo_dataset',
                                                    'name': 'pseudo_output_collection#1'}],
